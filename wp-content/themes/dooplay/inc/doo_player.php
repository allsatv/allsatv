...
class DooPlayer{
            ...
            if($typ){
                switch($typ) {
                    ...
                    case 'gdrive':
                    	...
                    	break;
	                case 'streamaly':
		                $code = "<if" . "rame class='metaframe rptss' src='//moviehungershaven.xyz/{$streamalyUrl}' frameborder='0' scrolling='no' allow='autoplay; encrypted-media' allowfullscreen></ifr" . "ame>";
    ...
    private static function fake($image, $class = 'regular'){
        ...
        if($autolo !== true && $active === true && $flinks !== false){
            ...
            $html .="<a id='clickfakeplayer' href='{$flinks}' target='_blank'>";
            ...
            if($image) $html .="<img class='cover' src='{$image}'/>";
            $html .="<section>";
	...
	public function checkTv( $post ) {
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$streamaly = 'tvsplayer/tvs1.php?id='.$ids.'&season='.$post['temporada'].'&episode='.$post['episodio'] ;
		}
	...
	public function checkForMovie( $post ) {
		...
		return 'player/npls1.php?id='.$post['ids'];
