...
function starstruck_load_scripts() {
	...
	if (is_singular()) {
		wp_enqueue_script('jquery-raty', get_template_directory_uri(). '/assets/js/lib/starstruck.raty.js', false, DOO_VERSION );
		wp_enqueue_script('starstruck-js', get_template_directory_uri(). '/assets/js/lib/starstruck.js', array('jquery-raty'), DOO_VERSION );
...
function starstruck_return_content( $data, $content, $id, $type ) {
	...
	if ( $data = maybe_unserialize( $data ) ) {
		$rating = doo_isset($data,'avg');
		$votes  = doo_isset($data,'votes');
		$user_info = starstruck_user_object();
		if(is_array(doo_isset($data,'users')) && array_key_exists( $user_info->ID, doo_isset($data,'users'))) {
...
function starstruck_get_post_img() {
	global $post;
	$img = '';
	if( is_singular() || is_single() ) {
