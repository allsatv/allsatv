    ...
    */
    ...
    public function validate_save( $request ) {
        ...
        if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
            ...
            $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'mtms' ), 'updated' );
        }
        // reset all options
        if ( isset( $request['resetall'] ) ) {
            $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'mtms' ), 'updated' );
            return;
        }
        // reset only section
        if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
            ...
            $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'mtms' ), 'updated' );
        }
    ...
    public function admin_page() {
        ...
        $maxwidth   = isset($this->settings['max_width']) ? " style='max-width: {$this->settings['max_width']}px'" : false;
        echo '<div class="cs-framework cs-option-framework" '.$maxwidth.'>';
        ...
        echo '<fieldset>';
        echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'mtms' ) .'</span>' : '';
        submit_button( esc_html__( 'Save changes', 'mtms' ), 'primary cs-save', 'save-header', false, array( 'data-save' => esc_html__( 'Saving...', 'mtms' ) ) );
        //submit_button( esc_html__( 'Restore', 'mtms' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
        if( $this->settings['show_reset_all'] ) {
            submit_button( esc_html__( 'Reset All Options', 'mtms' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
        }
        echo '</fieldset>';
        //echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'mtms' ) .'</a>' : '';
        ...
        foreach ( $this->options as $key => $tab ) {
            if((isset($tab['sections']))) {
                ...
                foreach ( $tab['sections'] as $tab_section ) {
                    ...
                    echo '<li><a href="#"'.$active_tab .' id="tab-'.$tab_section['name'].'" data-section="'.$tab_section['name'].'">'. $icon . $tab_section['title'] .'</a></li>';
                }
                echo '</ul>';
                echo '</li>';
            } else {
        ...
        echo '<div class="cs-block-left"><strong>DooPlay Options</strong> Powered by Codestar Framework.</div>';
        echo '<div class="cs-block-right">';
        submit_button( esc_html__( 'Save changes', 'mtms' ), 'primary cs-save', 'save-footer', false, array( 'data-save' => esc_html__( 'Saving...', 'mtms' ) ) );
        echo '</div>';
        echo '<div class="clear"></div>';
        echo '</footer>';
        echo '</form>';
